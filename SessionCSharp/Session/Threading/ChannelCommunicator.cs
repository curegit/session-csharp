...
namespace Session.Threading
{
	internal class ChannelCommunicator : ICommunicator
	{
		...
		public async Task<Session<S, Empty, P>> CastNewChannelAsync<S, P>() where S : SessionType where P : ProtocolType
		{
			var (c, s) = ChannelFactory.Create();
			await SendAsync(s);
			return new Session<S, Empty, P>(c);
		...
		public async Task<Session<S, Empty, P>> AcceptNewChannelAsync<S, P>() where S : SessionType where P : ProtocolType
		{
